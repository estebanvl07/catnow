...
export function MinimalLayout({ store, products, sections }: MinimalLayoutProps) {
  ...
  if (selectedSection === "all") {
    for (const section of sections) {
      const sectionProducts = filtered.filter((p) => p.section_id === section.id)
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="flex flex-col gap-8">
            {Array.from(groupedProducts.entries()).map(([sectionName, sectionProducts]) => (
              <div key={sectionName}>
                {sectionName && (
